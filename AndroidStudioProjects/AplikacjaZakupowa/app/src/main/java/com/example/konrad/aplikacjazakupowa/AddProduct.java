...
import static com.example.konrad.aplikacjazakupowa.NaszeMetody.shoppingList;
...
//                }catch (Exception e) {
//                    e.printStackTrace();
//                    List<Shopping> listaPomocnicza = new ArrayList<>();
//                    listaPomocnicza.add(new Shopping("Makarono", 1, "Kup w Makro"));
//                    listaPomocnicza.add(new Shopping(nameOfProduct.getText().toString(), Integer.valueOf(numberOfProducts.getText().toString()), Double.valueOf(priceOfProduct.getText().toString()), choiceBtn.isChecked(), true, "PLN", NaszeMetody.shoppingList.size()-1, additionalInfoEditText.getText().toString(), null, false, countedValue, countedValue2));
//                    NaszeMetody.shoppingList = listaPomocnicza;
//
//                }
                shoppingList.add(new Shopping(nameOfProduct.getText().toString(), Integer.valueOf(numberOfProducts.getText().toString()), Double.valueOf(priceOfProduct.getText().toString()), choiceBtn.isChecked(), true, "PLN", NaszeMetody.shoppingList.size()+1, additionalInfoEditText.getText().toString(), null, false, countedValue, countedValue2));
                sprawdzCzyJestDostep();
    ...
    public void sprawdzCzyJestDostep(){
            ...
            if(ActivityCompat.checkSelfPermission(AddProduct.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)
...
//            List<Shopping> shoppingList2  = new ArrayList<>();
//            zapiszDoPamieci(shoppingList2);
...
// zrobic ikone alpikacji
// zmienic nazwe aplikacji
// toasty z niewypelnieniem pol zrobic
// przed kliknieciem przycisku zrobic pola niewidoczne czyli gone
// inna kolejnosc: zrob swicha na samej gorze pod nazwa produktu
// nadpisz on back thereIsFirstVariableAccepted zeby wyswietlic alertdialoga czy chcesz wyjsc z aplikacji
// single line poprawic zeby nie nachodziła cena
// kolor checkboxa poprawic
// zawijac tlugi tekst
// popraw wynik koszyka
// wysylanie zrobic na liste send items w alert dialogu
// wysylanie smsow zeby uwzglednialo waluty
// dokonczyc wysylanie mailow
// po pierwszym wejsciu do aplikacji i kliknieciu go to list wyswietlaj liste z extal memory
// przy onPause zeby zapisywal liste do external memory
// getResource.getString(R.String.nazwa)
// on back pressed
...
// dodaje sie additional info do kazego produktu oprocz pierwszego zawsze

// kiedy nie trafie w przycisk apka staje
// nie potrafie ustawic przyciskow na srodku w toolbarze w shopping list

// za pierwszym razem jak wchodze do shopping list i pobieram liste z pamieci to apka staje . Dlatego lepiej za pierwszym razem wczytac liste automatycznie
